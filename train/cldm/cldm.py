...
class ControlLDM(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        color_block = batch['color_block'] # modify, load color block
        if bs is not None:
            control = control[:bs]
        control = control.to(self.device)
        control = einops.rearrange(control, 'b h w c -> b c h w')
        control = control.to(memory_format=torch.contiguous_format).float()
        return x, dict(c_crossattn=[color_block], c_concat=[control]) # modify, load color block
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        # modify, remove conditioning log
        ...
        if unconditional_guidance_scale > 1.0:
            # modify, get rid of empty string stuff
    ...
    # modify, re-write configure_optimizer, and add new function 'extractAttnParam(self, model)'
